...
import CreateBusinessHeads from "./create";
...
const BusinessHeads = () => {
  ...
  const fetchList = () => {
    setLoading(true)
    BusinessAccount.get({ page: Number(pageNumber) + 1 }).then((response) => {
  ...
  return (
    <>
      ...
      {
        deleteId &&
        <DeleteMoadal id={deleteId} handleOpen={setDeleteId} refresh={handleUpdate} statement={'Business Account'} />
      }

      <PageHeader title={'Business Accounts'} />

      <Card sx={{ p: 2, display: "flex", justifyContent: "end" }} variant="outlined">
        <CreateBusinessHeads key={refresh} onNew={handleCreate} onUpdate={handleUpdate} editId={editId} setEditId={setEditId} />
      </Card >
